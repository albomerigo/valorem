...
export function Search({ transactions, goals }: SearchProps) {
  ...
  const openSearch = useCallback(() => {
    ...
    if (rect) {
      setPanelPos({ top: rect.bottom + 8, left: rect.left, width: Math.max(rect.width, 400) });
    }
    setOpen(true);
    setQuery("");
    setActiveIndex(-1);
    ...
    <>
      ...
      <div
        style={{
          ...
          borderRadius: 16,
          boxShadow:
            "0 24px 48px -12px rgba(0,0,0,0.95), 0 8px 16px -4px rgba(168,139,250,0.3)",
          overflow: "hidden",
        ...
        {noResults ? (
          <div className="px-4 py-6 text-center">
            <p className="text-[12px] text-ink-secondary">
              ...
              <span className="text-ink-primary">&ldquo;{query}&rdquo;</span>
            </p>
          </div>
        ) : (
          <div className="py-2">
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                        isActive ? "bg-iri-violet/[0.12]" : "hover:bg-white/[0.03]"
                      }`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-ink-secondary">
                          {tx.category || "Altro"} · {formatDate(tx.transaction_date)}
                        </p>
                      </div>
                      <p
